...
% groups: data, optimizer, solver, MH_options, sampler, filter, postporcessing, output.
...
dynare_gui_.estimation.solver{num,3} = 'default | fixed_point';
...
num = num+1;
dynare_gui_.estimation.solver{num,1} = 'analytic_derivation';    
dynare_gui_.estimation.solver{num,2} =  '';    
dynare_gui_.estimation.solver{num,3} = 'check_option';   
dynare_gui_.estimation.solver{num,4} = 'Triggers estimation with analytic gradient. The final hessian is also computed analytically. Only works for stationary models without missing observations.';


num = num+1;
dynare_gui_.estimation.solver{num,1} = 'endogenous_prior';    
dynare_gui_.estimation.solver{num,2} =  '';    
dynare_gui_.estimation.solver{num,3} = 'check_option';   
dynare_gui_.estimation.solver{num,4} = 'Use endogenous priors as in Christiano, Trabandt and Walentin (2011).';

num = num+1;
dynare_gui_.estimation.solver{num,1} = 'use_univariate_filters_if_singularity_is_detected';    
dynare_gui_.estimation.solver{num,2} = '1';    
dynare_gui_.estimation.solver{num,3} = 'INTEGER';   
dynare_gui_.estimation.solver{num,4} = 'Decide whether Dynare should automatically switch to univariate filter if a singularity is encountered in the likelihood computation (this is the behaviour if the option is equal to 1). Alternatively, if the option is equal to 0, Dynare will not automatically change the filter, but rather use a penalty value for the likelihood when such a singularity is encountered. Default: 1.';
...
dynare_gui_.estimation.MH_options{num,3} = '[INTEGER1 INTEGER2 ...]';   
...
dynare_gui_.estimation.MH_options{num,3} = '[DOUBLE DOUBLE]';   
...
%% Group 5: sampler
num = 1;
dynare_gui_.estimation.sampler{num,1} = 'presample';    
dynare_gui_.estimation.sampler{num,2} = '0';    
dynare_gui_.estimation.sampler{num,3} = 'INTEGER';   
dynare_gui_.estimation.sampler{num,4} = 'The number of observations to be skipped before evaluating the likelihood. These first observations are used as a training sample. Default: 0.';     

num = num+1;
dynare_gui_.estimation.sampler{num,1} = 'mcmc_jumping_covariance';    
dynare_gui_.estimation.sampler{num,2} = 'hessian';    
dynare_gui_.estimation.sampler{num,3} = 'hessian | prior_variance | identity_matrix | FILENAME';   
dynare_gui_.estimation.sampler{num,4} = 'Tells Dynare which covariance to use for the proposal density of the MCMC sampler. mcmc_jumping_covariance can be one of the following: hessian,prior_variance,identity_matrix or FILENAME. Default value is hessian.';      
...
dynare_gui_.estimation.filter{num,2} = '';    
dynare_gui_.estimation.filter{num,3} = 'default | fixed_point | doubling | square_root_solver';   
...
%% Group 7: postporcessing
num = 1;
dynare_gui_.estimation.postporcessing{num,1} = 'bayesian_irf';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'check_option';   
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the computation of the posterior distribution of IRFs. The length of the IRFs are controlled by the irf option. Results are stored in oo_.PosteriorIRF.dsge (see below for a description of this variable).';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'dsge_var';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'DOUBLE';   
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the estimation of a DSGE-VAR model, where the weight of the DSGE prior of the VAR model is calibrated to the value passed (see Del Negro and Schorfheide (2004)). It represents ratio of dummy over actual observations. To assure that the prior is proper, the value must be bigger than (k + n)=T , where k is the number of estimated parameters, n is the number of observables, and T is the number of observations. NB: The previous method of declaring dsge_prior_weight as a parameter and then calibrating it is now deprecated and will be removed in a future release of Dynare.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'dsge_var_';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'check_option';   %special
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the estimation of a DSGE-VAR model, where the weight of the DSGE prior of the VAR model will be estimated (as in Adjemian et alii (2008)). The prior on the weight of the DSGE prior, dsge_prior_weight, must be defined in the estimated_params section. NB: The previous method of declaring dsge_prior_weight as a parameter and then placing it in estimated_params is now deprecated and will be removed in a future release of Dynare.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'dsge_varlag';    
dynare_gui_.estimation.postporcessing{num,2} = '4';    
dynare_gui_.estimation.postporcessing{num,3} = 'INTEGER';   
dynare_gui_.estimation.postporcessing{num,4} = 'The number of lags used to estimate a DSGE-VAR model. Default: 4.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'moments_varendo';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'check_option';   
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the computation of the posterior distribution of the theoretical moments of the endogenous variables. Results are stored in oo_.PosteriorTheoreticalMoments.The number of lags in the autocorrelation function is controlled by the ar option.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'ar';    %name
dynare_gui_.estimation.postporcessing{num,2} = '5';     %default value
dynare_gui_.estimation.postporcessing{num,3} = 'INTEGER';   %type
dynare_gui_.estimation.postporcessing{num,4} = 'Order of autocorrelation coefficients to compute and to print. Default: 5. Only useful in conjunction with option moments_varendo.';      

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'conditional_variance_decomposition';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = '[INTEGER1 INTEGER2 ...]';   
dynare_gui_.estimation.postporcessing{num,4} = 'Computes the posterior distribution of the conditional variance decomposition for the specified period(s). The periods must be strictly positive. Conditional variances are given by var(yt+kjt). For period 1, the conditional variance decomposition provides the decomposition of the effects of shocks upon impact. The results are stored in oo_.PosteriorTheoreticalMoments.dsge.ConditionalVarianceDecomposition, but currently there is no displayed output. Note that this option requires the option moments_varendo to be specified.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'filtered_vars';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'check_option';   
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the computation of the posterior distribution of filtered endogenous variables/one-step ahead forecasts, i.e. Etyt+1. Results are stored in oo_.FilteredVariables.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'smoother';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'check_option';   
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the computation of the posterior distribution of smoothed endogenous variables and shocks, i.e. the expected value of variables and shocks given the information available in all observations up to the final date (ET yt). Results are stored in oo_.SmoothedVariables, oo_.SmoothedShocks and oo_.SmoothedMeasurementErrors. Also triggers the computation of oo_.UpdatedVariables, which contains the estimation of the expected value of variables given the information available at the current date (Etyt).';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'selected_variables_only';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'check_option';   
dynare_gui_.estimation.postporcessing{num,4} = 'Only run the smoother on the variables listed just after the estimation command. Default: run the smoother on all the declared endogenous variables.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'forecast';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'INTEGER';   
dynare_gui_.estimation.postporcessing{num,4} = 'Computes the posterior distribution of a forecast on INTEGER periods after the end of the sample used in estimation. If no Metropolis-Hastings is computed, the result is stored in variable oo_.forecast and corresponds to the forecast at the posterior mode. If a Metropolis-Hastings is computed, the distribution of forecasts is stored in variables oo_.PointForecast and oo_.MeanForecast.';

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'filter_step_ahead';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = '[INTEGER1 INTEGER2 ...]';   
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the computation k-step ahead filtered values. Stores results in oo_.FilteredVariablesKStepAhead and oo_.FilteredVariablesKStepAheadVariances.';      

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'filter_decomposition';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'check_option';   
dynare_gui_.estimation.postporcessing{num,4} = 'Triggers the computation of the shock decomposition of the above k-step ahead filtered values.';      

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'irf';    
dynare_gui_.estimation.postporcessing{num,2} = '40';    
dynare_gui_.estimation.postporcessing{num,3} = 'INTEGER';   
dynare_gui_.estimation.postporcessing{num,4} = 'Number of periods on which to compute the IRFs. Setting irf=0, suppresses the plotting of IRFs. Default: 40. Only used if [bayesian irf] is passed.';      

num = num+1;
dynare_gui_.estimation.postporcessing{num,1} = 'irf_shocks';    
dynare_gui_.estimation.postporcessing{num,2} = '';    
dynare_gui_.estimation.postporcessing{num,3} = 'exo_var_list';   % special
dynare_gui_.estimation.postporcessing{num,4} = 'The exogenous variables for which to compute IRFs. Default: all. Only used if [bayesian irf] is passed. Cannot be used with [dsge var].'; 
...
dynare_gui_.estimation.output{num,3} = 'eps, pdf, fig, none';   
