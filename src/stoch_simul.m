...
if options_.irf
    ...
    % Modyfed by M.Labus
    num_shocks = 0;
    irfs_data = zeros(size(irf_shocks_indx,2),size(var_list,1),options_.irf); 
    % EndOfModification
            ...
            % EndOfModification
                ...
                elseif nbplt == 1
                    if options_.relative_irf
                        ...
                        % Modyfed by M.Labus
                        hh = dyn_figure(options_,'Name',[gui_tools.get_long_name(deblank(tit(i,:)),'shock') ' (' deblank(tit(i,:)) ')' cumulativeIRFString]);
                    ...
                    for j = 1:number_of_plots_to_draw
                        ...
                        %title(deblank(mylist(j,:)),'Interpreter','none');
                            ...
                            title(gui_tools.get_long_name(deblank(mylist(j,:)),'var'),'Interpreter','none');
                    ...
                    for fig = 1:nbplt-1
                        if options_.relative_irf
                            ...
                            hh = dyn_figure(options_,'Name',['Relative response to orthogonalized shock' ...
                                                ' to ' gui_tools.get_long_name(deblank(tit(i,:)),'shock') ' figure ' int2str(fig)]);    
                            
                                            
                            %hh = dyn_figure(options_,'Name',['Relative response to orthogonalized shock' ...
                                  %              ' to ' tit(i,:) ' figure ' int2str(fig)]);
                        else
                             hh = dyn_figure(options_,'Name',[gui_tools.get_long_name(deblank(tit(i,:)),'shock') ' (' deblank(tit(i,:)) ')' ...
                        ...
                        for plt = 1:nstar
                            ...
                            title(gui_tools.get_long_name(deblank(mylist((fig-1)*nstar+plt,:)),'var'),'Interpreter','none');
                            % EndOfModification
                        end
                    ...
                    % Modyfed by M.Labus
                    hh = dyn_figure(options_,'Name',[gui_tools.get_long_name(deblank(tit(i,:)),'shock') ' (' deblank(tit(i,:)) ')' cumulativeIRFString ' (' int2str(nbplt) ')']);
                    ...
                    for plt = 1:number_of_plots_to_draw-(nbplt-1)*nstar;
                        ...
                        %title(deblank(mylist((nbplt-1)*nstar+plt,:)),'Interpreter','none');
                              ...
                              title(gui_tools.get_long_name(deblank(mylist((nbplt-1)*nstar+plt,:)),'var'),'Interpreter','none');
    ...
    if(~displayByShock)
        ...
        for i = 1: nvar
            ...
            elseif nbplt == 1
                if options_.relative_irf
                    hh = dyn_figure(options_,'Name',['Relative response to' ...
                        ' orthogonalized shock on endogenous variable ' gui_tools.get_long_name(deblank(M_.endo_names(i_var(i),:)),'var') ' (' deblank(M_.endo_names(i_var(i),:)) ')']);
                else
                    hh = dyn_figure(options_,'Name',['Orthogonalized shocks on endogenous variable' ...
                        ' ' gui_tools.get_long_name(deblank(M_.endo_names(i_var(i),:)),'var') ' (' deblank(M_.endo_names(i_var(i),:)) ')' cumulativeIRFString]);
                end
                ...
                for j = 1:number_of_plots_to_draw
                    ...
                    xlim([1 options_.irf]);
                    % Modyfed by M.Labus
                    title([ gui_tools.get_long_name(deblank(tit(irfs_index(j),:)),'shock') ' (' deblank(tit(irfs_index(j),:)) ')'],'Interpreter','none');
                end
                %dyn_saveas(hh,[M_.fname '_IRF_' deblank(tit(i,:))],options_);
                    ...
                    for fig = 1:nbplt-1
                        if options_.relative_irf
                            hh = dyn_figure(options_,'Name',['Relative response to ' ...
                                ' orthogonalized shock on endogenous variable ' gui_tools.get_long_name(deblank(M_.endo_names(i_var(i),:)),'var') ...
                                ' (' deblank(M_.endo_names(i_var(i),:)) ')' ' figure ' int2str(fig)]);
                        else
                            hh = dyn_figure(options_,'Name',['Shocks on endogenous variable' ...
                        ' ' gui_tools.get_long_name(deblank(M_.endo_names(i_var(i),:)),'var') ' (' deblank(M_.endo_names(i_var(i),:)) ')' ...
                                cumulativeIRFString ' (' int2str(fig) ')']);
                        end
                        %set(hh, 'Position', [0 0 screen_size(3) screen_size(4) ] );
                        for plt = 1:nstar
                            ...
                            %title(gui_tools.get_long_name(deblank(mylist((fig-1)*nstar+plt,:)),'var'),'Interpreter','none');
                            %title([ gui_tools.get_long_name(deblank(tit(irfs_index((fig-1)*nstar+plt),:)),'shock') ' (' deblank(tit(irfs_index((fig-1)*nstar+plt),:)) ')'],'Interpreter','none');
                            title(gui_tools.get_long_name(deblank(tit(irfs_index((fig-1)*nstar+plt),:)),'shock'),'Interpreter','none');
                        end
                        %dyn_saveas(hh,[ M_.fname '_IRF_' deblank(tit(i,:)) int2str(fig)],options_);
                    ...
                    for plt = 1:number_of_plots_to_draw-(nbplt-1)*nstar;
                        ...
                        %title(gui_tools.get_long_name(deblank(mylist((nbplt-1)*nstar+plt,:)),'var'),'Interpreter','none');
                        %title([ gui_tools.get_long_name(deblank(tit(irfs_index((nbplt-1)*nstar+plt),:)),'shock') ' (' deblank(tit(irfs_index((nbplt-1)*nstar+plt),:)) ')'],'Interpreter','none');
                         title(gui_tools.get_long_name(deblank(tit(irfs_index((nbplt-1)*nstar+plt),:)),'shock'),'Interpreter','none');
