...
if ~isfield(project_info, 'mod_file')
    status = specify_file();
    if(status == 0)
        return;
    end
...
fullFileName = [project_info.project_folder,'\', project_info.mod_file];
load_file(fullFileName);
    ...
    function status = specify_file()
        ...
        if(strcmp([pathName,fileName],[project_folder,'\',fileName])~=1)
            
            [status, message] = copyfile([pathName,fileName],[project_folder,'\',fileName]);
            if(status)
