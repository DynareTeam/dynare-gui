...
try %M.Labus: fix for DATENUM bug - every second time DATENUM fails
...
elseif strcmp(indic,'backward_quarter')==1
    % dv(end,1)={rdate};
    dv(end,1)={datestr(datenum(rdate,'mmmyyyy'),'QQ-YYYY');};
    ...
    for t=2:p
        ...
        dv(p-t+1,1)={datestr([ty tm 1 0 0 0],'QQ-YY')};
    end
else
    error('Indicator must say ''forward'' or ''backward'' or ''backward_quarter''')
end