function irf_decomp(selected_vars, shock_id, shock_name, shock_value)
...
SS=Belox_GUI_oo.steady_state;  %114 x 1   ; sve varijable u deklarisanom redosledu
...
x(order_var)=  B(:,shock)*shock_value;
for j=1:num_vars
   decom(j,1,j) = x(j); 
end
...
for i=1:num_selected
    ...
    irf_graph_decomp(data_2,new_labels,var_name, shock_name);
end
