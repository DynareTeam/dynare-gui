...
% Modyfed by M.Labus
x = initial_date.first-1:initial_date.last;
z=z(:,:,initial_date.first:initial_date.last);
gend = size(z,3);
% EndOfModification
%% write LaTeX-Header
...
for j=1:nvar
    ...
    for i=1:gend
        ...
        for k = 1:comp_nbr
            zz = z1(k,i);
            if(k==comp_nbr)
                s_name = 'Initial values';
            else
                s_name = char(shock_names(k,:));
            end
            if zz > 0
                fill([x(i) x(i) x(i+1) x(i+1)],[yp yp+zz yp+zz yp],k, 'Tag', s_name); % Modyfed by M.Labus: 'Tag' value
                yp = yp+zz;
            else
                fill([x(i) x(i) x(i+1) x(i+1)],[ym ym+zz ym+zz ym],k, 'Tag', s_name); % Modyfed by M.Labus: 'Tag' value
                ym = ym+zz;
            end
            hold on;
        end
